...
for incat in ["astrefcat-3filt_only","astrefcat","astrefcat-stars_only","astrefcat-also_ng"]:
	...
	ooo=os.system("asctoldac -i %s.txt -o %s.cat -t LDAC_OBJECTS -c ~/wtgpipeline/asctoldac_panstarrs.config" % (incat,incat))
	if ooo!=0: raise Exception('asctoldac ran with os.system and got ERROR')
